...
def parse_sampled_batch(batch):
    ...
    item_indices = torch.where(pos != 0)
    nbrs_indices = torch.where(nbr != 0)
    return batch, item_indices, nbrs_indices
...
def train(model, opt, shdlr, train_loader, args):
    ...
    for batch in train_loader:
        parsed_batch, item_indices, nbrs_indices = parse_sampled_batch(batch)
        opt.zero_grad()
        shdlr.step()

        pos_logits, neg_logits, nbr_logits, user_emb, pos_hi, neg_hi, nbr_emb = model(parsed_batch)

        # Label Loss
        loss = 0.0
        if args.loss_type == 'bce':
            loss += F.binary_cross_entropy_with_logits(pos_logits[item_indices], torch.ones_like (pos_logits)[item_indices]) + \
                    F.binary_cross_entropy_with_logits(neg_logits[item_indices], torch.zeros_like(neg_logits)[item_indices])
        elif args.loss_type == 'bpr':
            # loss += -1.0 * F.logsigmoid(pos_logits[indices] - neg_logits[indices]).mean()
            loss += F.softplus(neg_logits[item_indices] - pos_logits[item_indices]).mean()

        # Embedding Reg loss
        user_norm = user_emb.norm(2, dim=-1).pow(2).mean() + nbr_emb.norm(2, dim=-1).pow(2).mean()
        item_norm = pos_hi.norm(2, dim=-1).pow(2).mean() + neg_hi.norm(2, dim=-1).pow(2).mean()
        emb_reg_loss = args.emb_reg * (user_norm + item_norm) / 2
        loss += emb_reg_loss

        # Social Reg term
        loss += args.alpha * F.binary_cross_entropy_with_logits(nbr_logits[nbrs_indices], torch.ones_like(nbr_logits)[nbrs_indices])
...
def main():
    parser = argparse.ArgumentParser(description='SocRec')
    parser.add_argument('--model', default='SocRec')
    ...
    for r in range(args.repeat):
        ...
        model = SocRec(user_num, item_num, args)
